use flight_builder_core::{Res, ResMut, Schedule, Scheduler};
use rand::random;
...
fn print_readings(readings: Res<SensorReadings>){
    println!("Bar: {} Temp: {} Humidity: {}", readings.bar, readings.temperature, readings.humidity);
}
...
fn main() {
    ...
    s.add_resource(SensorReadings{bar:0.0, temperature:0.0, humidity:0.0});
